-- 1. Add subscription lifecycle columns to group_purchases
...
CREATE INDEX IF NOT EXISTS idx_group_purchases_subscription_id
  ON public.group_purchases (stripe_subscription_id);
CREATE INDEX IF NOT EXISTS idx_group_purchases_group_status
  ON public.group_purchases (group_id, status);

-- 2. Replace user_has_video_access so group access requires an active, unexpired group_purchase
...
BEGIN
  ...
  -- Group access: requires the group_purchase to still be active (or trialing) AND not expired.
  IF EXISTS (
    SELECT 1
